...
static const GColor s_color_table[] = {
    ...
    GColorCyan, GColorYellow, GColorMagenta, GColorWhite, GColorLightGray
};
...
static void update_step_state(void) {
    if (!s_settings.show_step_dots) return;
    int32_t steps = 0;
...
static void update_health_state(void) {
    ...
    if (s_settings.show_activity_dot) {
        ...
        s_activity_active = (activities & HealthActivityWalk)
                         || (activities & HealthActivityRun);
    }
#endif
}
...
// Called at init only — populates all time/date/angle state in one localtime() call
static void update_time_state(void) {
    ...
    s_month      = t->tm_mon + 1;
    s_weekday    = t->tm_wday;
    s_hour_angle = deg_to_trig(((t->tm_hour % 12) * 60 + t->tm_min) * 360 / (12 * 60));
...
static void canvas_update_proc(Layer *layer, GContext *ctx) {
    ...
    graphics_context_set_fill_color(ctx,
        s_connected ? get_color(s_settings.bt_color_idx) : GColorRed);
    ...
    if (s_settings.show_battery_dots) {
        // Color progression: charging=Cyan, 4-5 dots=White, 3=Yellow, 2=Orange, 1=Red
        ...
        for (int i = 0; i < 5; i++) {
            graphics_context_set_fill_color(ctx,
                i < 5 - s_battery_dots ? GColorDarkGray : lit_color);
            graphics_fill_circle(ctx,
                point_on_circle(center, deg_to_trig(r_ang + i * SPACING_DEG), s_outer_r), 2);
        }
    ...
    if (s_settings.show_alarm_dot) {
        graphics_context_set_fill_color(ctx,
            s_alarm_pending ? get_color(s_settings.alarm_color_idx) : GColorDarkGray);
        graphics_fill_circle(ctx, point_on_circle(center, deg_to_trig(r_ang), s_outer_r), 2);
        r_ang += GAP2_DEG;
    }

    if (s_settings.show_notif_dot) {
        GColor c = s_notif_count == 0                          ? GColorDarkGray
                 : s_notif_count >= s_settings.notif_threshold ? get_color(s_settings.notif_alert_color_idx)
    ...
    if (s_settings.show_event_dot) {
        graphics_context_set_fill_color(ctx,
            s_event_pending ? get_color(s_settings.event_color_idx) : GColorDarkGray);
        graphics_fill_circle(ctx, point_on_circle(center, deg_to_trig(r_ang), s_outer_r), 2);
        r_ang += GAP2_DEG;
    }
    ...
    if (s_settings.show_hr_dot) {
        GColor c = s_heart_rate == 0                              ? GColorDarkGray
                 : s_heart_rate >= s_settings.hr_alert_bpm        ? get_color(s_settings.hr_alert_color_idx)
    ...
    if (s_settings.show_activity_dot) {
        graphics_context_set_fill_color(ctx,
            s_activity_active ? get_color(s_settings.activity_color_idx) : GColorDarkGray);
        graphics_fill_circle(ctx, point_on_circle(center, deg_to_trig(r_ang), s_outer_r), 2);
    }
    ...
    if (s_settings.show_step_dots) {
        ...
        for (int i = 0; i < 10; i++) {
            graphics_context_set_fill_color(ctx, i < s_step_lit ? step_c : GColorDarkGray);
            graphics_fill_circle(ctx,
                point_on_circle(center, deg_to_trig(225 - i * SPACING_DEG), s_outer_r), 2);
        }
    }
    ...
    if (s_settings.show_month_dots) {
        for (int i = 0; i < 4; i++) {
            ...
            graphics_fill_circle(ctx,
                point_on_circle(center, deg_to_trig(l_ang - i * SPACING_DEG), s_outer_r), 2);
        }
    ...
    if (s_settings.show_weekday_dot) {
        graphics_context_set_fill_color(ctx, get_color(s_settings.weekday_color_idx[s_weekday]));
        graphics_fill_circle(ctx, point_on_circle(center, deg_to_trig(l_ang), s_outer_r), 2);
    }
    ...
    graphics_draw_line(ctx, center, point_on_circle(center, s_hour_angle, s_hour_len));
    graphics_context_set_stroke_color(ctx, get_color(s_settings.minute_color_idx));
    graphics_draw_line(ctx, center, point_on_circle(center, s_min_angle, s_min_len));
}
...
#define APPLY_BOOL(key, field)  t = dict_find(iter, key); if (t) s_settings.field = t->value->uint8 != 0;
#define APPLY_U8(key, field)    t = dict_find(iter, key); if (t) s_settings.field = t->value->uint8;
#define APPLY_I32(key, field)   t = dict_find(iter, key); if (t) s_settings.field = t->value->int32;
...
#undef APPLY_I32
    ...
    for (int day = 0; day < 7; day++) {
        ...
        if (t) s_settings.weekday_color_idx[day] = t->value->uint8;
    }

    t = dict_find(iter, MESSAGE_KEY_NotifCount);
    if (t) s_notif_count = t->value->int32;

    t = dict_find(iter, MESSAGE_KEY_AlarmPending);
    if (t) s_alarm_pending = t->value->uint8 != 0;

    t = dict_find(iter, MESSAGE_KEY_EventPending);
    if (t) s_event_pending = t->value->uint8 != 0;
...
static void unobstructed_change(AnimationProgress progress, void *context) {
    compute_geometry(layer_get_unobstructed_bounds(window_get_root_layer(s_window)));
    layer_mark_dirty(s_canvas);
}
